...
export default function Hero() {
  ...
  // Fonction pour scroller proprement jusqu'à la discographie
  const scrollToDiscography = (e: React.MouseEvent) => {
    e.preventDefault();
  ...
  return (
    <section className="relative h-[80vh] flex flex-col items-center justify-center text-center bg-zinc-950 text-white overflow-hidden">
      ...
      <div className="z-10 mt-12 flex gap-4">
        {/* CHANGEMENT ICI : On utilise un bouton avec scrollToDiscography */}
        <button 
          onClick={scrollToDiscography}
          className="px-8 py-3 bg-white text-black font-bold rounded-full hover:bg-zinc-200 transition cursor-pointer"
        >
          Dernier Release
        </button>
        ...
        <button 
          ...
          className="px-8 py-3 border border-white/20 hover:bg-white/10 transition rounded-full font-bold cursor-pointer"
        >
          Contact & Plateformes
        </button>
      </div>
      ...
      {showContact && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
          <div className="relative bg-zinc-900 border border-white/10 p-8 rounded-3xl max-w-sm w-full">
            ...
            <h2 className="text-2xl font-bold mb-6 text-center text-white">Retrouve Kyzzo</h2>
